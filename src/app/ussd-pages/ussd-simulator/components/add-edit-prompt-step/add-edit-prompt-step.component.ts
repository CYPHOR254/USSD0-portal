...
export class AddEditPromptStepComponent implements OnInit {
  ...
  submitNewStep() {
    console.log("submitNewStep called");
    ...
    console.log("New Step Object:", newStepObj);
    ...
    if (this.basicForm.controls["type"].value === "select") {
        ...
        if (this.selectedOptionType === "manual") {
            ...
            newStepObj["options"] = this.optionsForm.controls["existingOption"].value;
            ...
            newStepObj["options-error"] = this.basicForm.controls["name"].value.concat("-options-error");

            this.allJSON["language"][this.data["promptName"] as string][lang][newStepObj["options-error"] as string] = this.optionsForm.controls["optionError"].value;
            newStepObj["next"] = this.navigateForm.controls["next"].value;
        } else {
            this.toastrService.warning("Invalid option type");
            console.log("Invalid option type selected");
        ...
        if (this.validationParamsArr?.length > 0 && this.validationStatus === true) {
            newStepObj["validation"] = this.validationParamsArr;
        }
    } else if (this.basicForm.controls["type"].value === "skip") {
        newStepObj["previous"] = this.navigateForm.controls["previous"].value;
        newStepObj["next"] = this.navigateForm.controls["next"].value;
        ...
        this.toastrService.info("Invalid option type");
        console.log("Type is 'skip'");
        return; // Exit the function early if the type is neither "select" nor "input" nor "skip"
    }

    if (this.navigateForm.controls["save-as"].value !== "" && this.navigateForm.controls["save-as"].value !== null) {
        newStepObj["save-as"] = this.navigateForm.controls["save-as"].value;
    }

    if (this.navigateForm.value.hasApi === true) {
        let externalFetchObj: any = {
        ...
        if (this.cacheStatus) {
            ...
            externalFetchObj["cache-path"] = this.advancedForm.controls["cache-path"].value;
            externalFetchObj["cache-parameters"] = this.cacheParamsArr;
        }
        newStepObj["external-fetch"] = externalFetchObj;
    ...
    console.log("Updated Prompt Data:", this.promptData);

    this.allJSON["prompts"][this.data["promptName"] as string] = this.promptData;

    if (this.basicForm.controls["type"].value !== "skip") {
        this.allJSON["language"][this.data["promptName"] as string][lang][newStepObj["name"] as string] = this.basicForm.controls["title"].value;
    }

    this.allJSON["language"][this.data["promptName"] as string][lang][newStepObj["error"] as string] = this.basicForm.controls["errorMsg"].value;

    let tempArr = [...this.allJSON["prompts_cache"][this.data["promptName"] as string]];
    tempArr.push(formattedStepName);

    this.allJSON["prompts_cache"] = Object.assign(this.allJSON["prompts_cache"], {
        [this.data["promptName"] as string]: tempArr,
    });

    console.log("Final JSON Data:", this.allJSON);
  ...
  //     }
  //   } else {
  //     this.toastrService.info("Skipping type");
  //     console.log("Type is 'skip'");
